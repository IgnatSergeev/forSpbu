...
void readGraphEdgesAndCapitalsAndAssignCitiesToTheCountries(FILE *file, Graph *graph, int roadsNumber, int citiesNumber, int *numberOfCapitals) {
    for (int i = 0; i < roadsNumber; i++) {
        ...
        startCityIndex -= 1;
        endCityIndex -= 1;
        addEdge(graph, startCityIndex, endCityIndex, length);
    }

    fscanf(file, "%d", numberOfCapitals);
    int **countriesClosestNodesLists = calloc(*numberOfCapitals, sizeof(int *));
    ...
    for (int i = 0; i < *numberOfCapitals; i++) {
        ...
        int *closestNodeList = calloc(citiesNumber, sizeof(int));
        for (int j = 0; j < citiesNumber; j++) {
            closestNodeList[j] = INT_MAX;
        }
        closestNodeList[capitalIndex] = 0;
        countriesClosestNodesLists[i] = closestNodeList;
    }
    ...
    for (int i = 0; i < *numberOfCapitals; i++) {
        free(countriesClosestNodesLists[i]);
    }
...
int main(void) {
    ...
    readGraphEdgesAndCapitalsAndAssignCitiesToTheCountries(file, graph, roadsNumber, citiesNumber, &numberOfCapitals);
    fclose(file);
    for (int i = 0; i < n) {
